...
## setter 与getter 方法
　　让外部对象对该类数据有统一的访问接口,因此可以在setter中,进行验证设置,看是否可以进行值的设置;
getter 方法 可以根据外部对象某些值的,确定是否可以进行值的获取,或者实现惰性求值(需要用的时候才进行求值,因为某些变量的求解的时间较长)
  安全
  
  ## 三元运算符
 `变量=返回值2 if condition else 返回值1`
