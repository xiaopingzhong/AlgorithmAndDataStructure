...
class TestAdd2(TestCase):
    ...
    def test_add(self):
        ...
        add_result = add(1, 2)
        # assert 表达式,断言错误信息
        self.add_judge_result(add_result)

    def method_name(self, add_result, add_error_info_parameter=add_error_constant):
        """
        主要目的就是测试加法
        Args:
            add_result:
            add_error_info_parameter:

        Returns:
            空
        """
        assert add_result == 3, add_error_info_parameter

    def add_judge_result(self, add_result, add_error_parameter_2="相加结果正确", result_mark="1"):
        assert add_result == 3, "%s" % add_error_parameter_2
        print("value", sep=" ", end=":")
